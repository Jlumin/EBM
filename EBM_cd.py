...
import struct
import binascii
#portName = "/dev/ttyUSB0"    # Raspberry Pi 3
portName = "COM1"
...
try:
    while ser.isOpen:
        ...
        saveFid = "test_data/acc_read_{}.bin".format(now)
        ...
        with open(saveFid, "a") as file:
            ...
            for ii in range(0, len(dataAcc_save)):
                file.write(str(format(dataAcc_save[ii][0], '0.3f')))
                file.write(",")
                file.write(str(format(dataAcc_save[ii][1], '0.3f')))
                file.write(",")
                file.write(str(format(dataAcc_save[ii][2], '0.3f')))
                file.write(",\n")
